...
AI transcript committed inside this repository; CI run history.
...
|---|---|---|---|
| `SYN-C01` | Microphone permission taken off page load | E1 — one-line removal traced to commit `75d06fc`, with a stated need predating it (`AUTOPSY-P2-01` → `Faz2.4`) | E3 **NOT_OBSERVED**. The ledger row reads `verified`, but its own detail cell records a code trace substituted for the browser test, "pending live browser confirmation by the user". |
...
**Held back:** the 2026-09-20 database permission findings. Consequential, but the
underlying state arrived as SQL output pasted into a chat; only the resulting
migrations and README text are in the repo, and the captured values cannot be
...
| V-03 | Application runtime | **NOT_OBSERVED** — nothing was launched. This review produced no E3 evidence of its own. |
...
1. Was the cleared-permissions browser check for `SYN-C01` ever performed? If so,
   when, and on which browser/OS? Nothing in the repository answers this, and it
   is the difference between E1 and E3 for the strongest candidate.
2. Has any CI run ever completed successfully? The workflow exists; execution was
   not verifiable here.
...
Obtain answers to questions 1 and 2 from the author — both are manual checks
outside the repository — then re-level `SYN-C01` and the CI claim in
[`../EVIDENCE_INDEX.md`](../EVIDENCE_INDEX.md). Until then `SYN-C01` stays at E1
with E3 `NOT_OBSERVED`, and the CI claim stays E0.

Separately, the hub should decide whether an AutoSRT pass is commissioned; that
project sits beside this one on the same machine and has not been reviewed.
...
Modified: none. `Planner-docs/` was read and linked, never edited or copied.
